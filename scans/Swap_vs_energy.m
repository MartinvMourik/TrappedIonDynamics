...
scan_vals = logspace(log10(40),log10(90),15);
...
settings.rf_voltage = 23.6;
settings.rf_frequency = 32.35e6;
settings.rf_phase = 0;
...
for i = 1:length(ions)
    ...
    settings.ions(i).coupling = 5*2*pi*1e6;
...
for i = 1:length(scan_vals)
    for j = 1:number_of_repeats
        ...
        settings.ions(1).start_vel = rand_direction*scan_vals(i);
